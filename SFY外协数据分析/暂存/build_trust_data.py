...
def calculate_common_features(ai_control_time, person_control_time, ai_time=None, person_time=None):
    """
    通用函数：计算共同的特征值
    """
    # 计算AI时间比例
    if ai_time is not None and person_time is not None:
        # 对于threat和sensor任务
        if pd.notna(ai_time) and pd.notna(person_time) and float(person_time) > 0:
            ai_time_ratio = float(ai_time) / float(person_time)
        else:
            ai_time_ratio = 0.0
    else:
        # 对于weapon和platform任务
    ...
    for _, record in weapon_records.iterrows():
        ...
        for _, weapon_row in weapon_data.iterrows():
            ...
            if pd.notna(current_times) and pd.notna(ai_remind_time):
                accept_ai_advice = float(ai_remind_time)/float(current_times)
            else:
...
def extract_threat_features(trust_conn, source_conn):
    ...
    for _, record in threat_records.iterrows():
        ...
        for _, threat_row in threat_data.iterrows():
            ...
            ai_time = threat_row['ai_select_time']
            person_time = threat_row['person_select_time']
            ai_target = threat_row['ai_target_id']
            person_target = threat_row['person_target_id']
            
            # 计算控制时间
            ai_control_time = float(ai_time) if pd.notna(ai_time) else 0.0
            if pd.notna(person_time) and pd.notna(ai_time):
                person_control_time = float(person_time) - float(ai_time)
            else:
                person_control_time = 0.0
            
            # 计算特征
            ai_time_ratio, person_dominance = calculate_common_features(
                ai_control_time, person_control_time, ai_time, person_time
            )
...
def extract_sensor_features(trust_conn, source_conn):
    ...
    for _, record in sensor_records.iterrows():
        ...
        for _, sensor_row in sensor_data.iterrows():
            ...
            ai_time = sensor_row['ai_select_time']
            person_time = sensor_row['person_select_time']
...
def build_trust_data():
    ...
    """
    # 连接到源数据库
    source_conn = sqlite3.connect('all_data.db')
    ...
    # 创建trust_data数据库
    trust_conn = sqlite3.connect('valid_data.db')
    ...
    print("✅ valid_data数据库trust_data表格构建完成！")
    print(f"传感器任务: {sensor_count} 条数据")
    print(f"威胁排序任务: {threat_count} 条数据") 
    print(f"平台控制任务: {platform_count} 条数据")
    print(f"武器发射任务: {weapon_count} 条数据")
