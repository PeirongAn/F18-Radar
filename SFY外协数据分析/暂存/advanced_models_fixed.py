...
def train_models_with_sampling(X, y, task_name):
    """使用不同采样方法训练模型"""
    print(f"\n=== 训练 {task_name} 任务模型 ===")
        ...
        try:
            ...
            for model_name, model in models.items():
                try:
                    ...
                    print(f"  {model_name} 失败: {str(e)[:50]}...")
                    continue
            
            # 找出最佳模型
            if method_results:
                best_model_name = max(method_results.keys(), key=lambda x: method_results[x]['accuracy'])
                best_results[method_name] = {
                    'best_model': best_model_name,
                    'accuracy': method_results[best_model_name]['accuracy'],
                    'all_results': method_results
                }
                print(f"最佳模型: {best_model_name}, 准确率: {method_results[best_model_name]['accuracy']:.4f}")
        
        except Exception as e:
            print(f"{method_name} 处理失败: {str(e)[:50]}...")
            continue
...
def main():
    ...
    for task in tasks:
        ...
        # 训练任务特定模型
        task_results = train_models_with_sampling(X_task, y_task, task)
    ...
    global_results = train_models_with_sampling(X, y, "全局")
    ...
    for task, results in all_task_results.items():
        ...
        if results:
            for method, result in results.items():
                ...
                print(f"{method}: {model_name} - 准确率: {accuracy:.4f}")
                
                if accuracy > best_overall_accuracy:
                    best_overall_accuracy = accuracy
                    best_overall_task = task
    ...
    print(f"\n🏆 全局最佳结果:")
    print(f"任务: {best_overall_task}")
    print(f"方法: {best_overall_method}")
    print(f"模型: {best_overall_model}")
    print(f"准确率: {best_overall_accuracy:.4f}")
    
    if best_overall_accuracy >= 0.9:
        print("🎉 成功达到90%+准确率目标!")
    else:
        print(f"⚠️  未达到90%目标，当前最佳: {best_overall_accuracy:.4f}")
        print("建议:")
        print("1. 收集更多数据")
        print("2. 改进特征工程")
        print("3. 尝试深度学习模型")
