...
def main():
    ...
    parser.add_argument('--file1', default="fold_results_20250908_191520.json", 
                       help='第一个fold_results文件路径')
    parser.add_argument('--file2', default="fold_results_20250909_110628.json", 
                       help='第二个fold_results文件路径')
    parser.add_argument('--visualize', action='store_true', 
                       help='是否生成可视化图表')
    parser.add_argument('--output-dir', default="svm_analysis_plots", 
                       help='可视化图表输出目录')
    
    args = parser.parse_args()
    
    # 指定要处理的两个文件
    fold_results_file1 = args.file1
    fold_results_file2 = args.file2
    ...
    # 检查文件是否存在
    if not os.path.exists(fold_results_file1):
        print(f"文件不存在: {fold_results_file1}")
        return
    
    if not os.path.exists(fold_results_file2):
    ...
    if metrics1 is not None and metrics2 is not None:
        ...
        # 生成超参数热力图
        print(f"\n=== 生成超参数热力图 ===")
        hyperparameter_file = "hyperparameter_search_results_20250908_191520.csv"
