...
def prepare_training_data_with_augmentation(df):
    ...
    try:
        ...
        db_filename = f"augmented_data_{timestamp}.db"
...
def hyperparameter_tuning(X, y_data_label):
    ...
    try:
        ...
        # 保存完整结果到CSV文件（带时间戳）
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        csv_filename = f"hyperparameter_search_results_{timestamp}.csv"
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        best_params_filename = f"best_hyperparameters_{timestamp}.json"
...
def train_and_evaluate_with_best_params(X_scaled, y_data_label, best_params):
    ...
    try:
        ...
        # 保存fold级详细结果（带时间戳）
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        fold_filename = f"fold_results_{timestamp}.json"
...
def visualize_results(results, selected_tasks):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig('multi_task_svm_results.png', dpi=300, bbox_inches='tight')
        plt.show()
